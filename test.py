...
def e2e(filename, kie='rule', save_image=False):
    try:
        ...
        if kie == 'pick':
            final_dict, draw_image, module_time = predictor.predict(img, filename, 'pick', save_image)
        else:
            final_dict, draw_image, module_time = predictor.predict(img, filename, 'rule', save_image)
...
if __name__ == '__main__':
    ...
    if module_name == 'detection':
    ...
    elif module_name == 'e2e_rule' or module_name == 'e2e_pick':
        ...
        filename_list = [i for i in os.listdir(E2E_DATA_DIR) if os.path.splitext(i)[1] == '.jpg']
        filename_list.sort()
        all_module_time = {
            'preprocess': 0,
            'detection': 0,
        ...
        for filename in filename_list:
            print('e2e ', filename)
            if module_name == 'e2e_rule':
                module_time = e2e(filename, 'rule', save_image)
            else:
                module_time = e2e(filename, 'pick', save_image)
            if module_time != None:
                for k in all_module_time:
                    all_module_time[k] += module_time[k]
        print(all_module_time)
        for k in all_module_time:
            print('Average time for {}: {}'.format(k, all_module_time[k]/len(filename_list)))
